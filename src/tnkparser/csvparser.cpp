...
void CsvParser::open(const QString &path)
{
    ...
    if(!m_inFile->open(QIODevice::Text | QIODevice::ReadOnly))
        return;
...
BaseParser::RowData CsvParser::nextRow()
{
    ...
    QString line = m_codec->toUnicode( m_inFile->readLine());
    ...
    {        
        data[m_headers.at(i)] = split.at(i).trimmed();
    }
...
void tnk::parser::CsvParser::selectTable(const QString &tableName, int headerLine)
{
    ...
    auto ba = m_inFile->readLine();
   m_codec = QTextCodec::codecForUtfText( ba);

   QString line = m_codec->toUnicode( ba);
