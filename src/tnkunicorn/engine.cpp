...
void Engine::create(QList<Entity*> *entities)
{
    ...
    {
        columns << QString("%1").arg(meta->property(i).name());
    ...
    {
        entity->set_created(QDateTime::currentDateTime());
        for(int i=2; i< meta->propertyCount(); i++)
        {
            if(QString(meta->property(i).typeName()).right(1) == "*")
            {
                ...
                if(sub)
                {
                    sub->get_id().isValid() ? this->update(sub) : this->create( sub);
                    id = sub->get_id();
                }
...
void Engine::update(QList<Entity *> *entities)
{
    ...
    foreach( auto entity, *entities)
    {
        entity->set_updated(QDateTime::currentDateTime());
        for(int i=1; i< meta->propertyCount(); i++)
        {
            if(QString(meta->property(i).typeName()).right(1) == "*")
            {
...
void Engine::setLastError(Error::Type type, const QString &text, const QString &sqlText)
{
...
bool Engine::findSqlError(const QString &text, const QSqlError &error)
{
    if( error.isValid() == false)
        return false;

    setLastError( Error::ERR_SQL, text, error.databaseText() + "-" + error.driverText());
    return true;
}
...
Error Engine::getLastError() const
{
    return _lastError;
}
