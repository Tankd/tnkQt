...
void JsonObject::fromData(const QByteArray &data)
{
    m_data = QJsonDocument::fromJson( data).object();
}

void JsonObject::fromValue(const QJsonValue &value)
{
    m_data = value.toObject();
}
...
void JsonObject::loadConfig()
{
    load( QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) +"/config.json");
}

void JsonObject::saveConfig()
{
    save( QStandardPaths::writableLocation(QStandardPaths::AppConfigLocation) +"/config.json");
}

QJsonValue JsonObject::value(const QString& path)
{
...
void JsonObject::setValue( const QString& path, const QJsonValue& value)
{
    if( path.contains("["))
...
void JsonObject::setValue(QJsonObject *object, const QString& path, const QJsonValue& value)
{    
    QStringList paths = path.split("/");
